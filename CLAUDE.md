...
When executing skills, use the following specialized agents:

| Skill            | Agent                    | Purpose                                      |
|------------------|--------------------------|----------------------------------------------|
| `auth-skill`     | `auth-security`          | Authentication, Better Auth, JWT, user mgmt  |
| `frontend-skill` | `nextjs-frontend-dev`    | Next.js App Router pages, components, UI     |
| `database-skill` | `neon-postgres-manager`  | Neon PostgreSQL schema, queries, migrations  |
| `backend-skill`  | `fastapi-backend`        | FastAPI endpoints, Pydantic models, SQLModel |

**Routing rules:**
- When `auth-skill` is invoked or auth work is needed → launch `auth-security` agent
- When `frontend-skill` is invoked or frontend work is needed → launch `nextjs-frontend-dev` agent
- When `database-skill` is invoked or database work is needed → launch `neon-postgres-manager` agent
- When `backend-skill` is invoked or backend work is needed → launch `fastapi-backend` agent

## Authentication Architecture (Better Auth + JWT)

Better Auth is configured to issue JWT tokens when users log in. These tokens are self-contained credentials that include user information and can be verified by any service that knows the shared secret key.
...
### Auth Constraints
- JWT shared secret must be stored in `.env`, never hardcoded
- All protected API routes must validate the Bearer token before processing
- User data isolation: queries must always filter by authenticated user ID
- Token expiration and refresh must be handled by Better Auth on the frontend
...
## Active Technologies
- Python 3.10+ + `mcp[cli]` (Official MCP SDK), SQLModel, python-dotenv (005-mcp-server-task-tools)
